...
func NewMessageService(cr *repository.ConfigRepository) *MessageService {
...
func (s *MessageService) SendMessage(ctx context.Context, b *bot.Bot, chatID int64, message string) {
	config, err := s.configRepo.FindConfigByChatID(ctx, nil, chatID)
	if err != nil {
		logger.Error(fmt.Sprintf("MessageService.SendMessage: %v", err))
		return
	}
